...
def create_url_params_list(location, term, limit):
	...
	if limit <= max_results_per_request:
		...
		extra_params = [{'term': term.replace(' ', '+'), 'location': location.replace(' ', '+'), 'limit': len(request), 'offset': request[0] + 1} for request in splits]
		return extra_params
...
def main():
	...
	parser.add_argument('-q', '--term', dest='term', default=DEFAULT_TERM, type=str, help=f'Search term, e.g. {DEFAULT_TERM}.')
	parser.add_argument('-l', '--location', dest='loc', default=DEFAULT_LOCATION, type=str, help=f'Search location, e.g. {DEFAULT_LOCATION}.')
	parser.add_argument('-li', '--limit', dest='lim', default=DEFAULT_SEARCH_LIMIT, type=int, help=f'Search limit, e.g. 3')
	parser.add_argument('-s', '--save', dest='save', type=str2bool, nargs='?', const=True, default=False, help='Save to a local file. If false just prints data to stdout.')
